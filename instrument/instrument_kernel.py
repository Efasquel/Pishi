...
def target_function():
    ...
    for taged_function in tagged_functions:
        ...
        kc_function_address = kc_kerne_start_address.add(
            k_off
        )  # if function size if 4 then add next address as function
        kc_function_address_end = kc_function_address.add(
            int(taged_function[FUNC_SIZE])
        )
...
def generate_assembly_instructions(x64_number):
    if not isinstance(x64_number, str) or len(x64_number) > 16:
        raise ValueError(
            "Input must be a 64-bit hexadecimal number as a string, e.g., 'fffffffffffffffe'"
        )
...
def kext_index_to_kext_flag(kext_index):
    ...
    if (
        kext_index > 16
    ):  # we use one mov instruction, the max immediate values on aarch64 is 16 bit.
...
def main():
    ...
    if not all_basic_blocks:
        print(
            "all_basic_blocks is empty check if start_address and end_address is correct."
        )
    ...
    for function_blocks in all_basic_blocks:
        for block in function_blocks:
            ...
            if patch_address == None:
                continue
            current_address = stub_gen.instrument(
                current_address,
                patch_address,
                original_opcode,
                needs_fix,
                bb_index,
                kext_index_to_kext_flag(1),
            )
            bb_index = bb_index + 1
